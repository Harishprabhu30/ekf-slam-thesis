...
def load_phase_segments(phase_csv: str, t_end_s: float) -> list:
    """
    Build segments as [(phase_id, t0_s, t1_s), ...] using /traj_phase events.
    IMPORTANT: We anchor time to the first occurrence of phase==1 (logical start),
    not motion onset, to avoid negative times and segment collapse.
    """
    phases = pd.read_csv(phase_csv).sort_values("t_ns").reset_index(drop=True)
    if phases.empty:
        raise RuntimeError(f"Phase CSV '{phase_csv}' is empty")

    # Anchor on phase 1 (logical trajectory start)
    ...
    # Convert to aligned seconds relative to phase 1
    phases["t_s"] = (phases["t_ns"].astype(np.int64) - t0_anchor_ns) * 1e-9
    phases = phases.sort_values("t_s").reset_index(drop=True)

    # Optional: if duplicate phase events occur, keep first occurrence of each phase
    ...
    for i in range(len(phases)):
        ...
        t1 = float(phases.loc[i + 1, "t_s"]) if i + 1 < len(phases) else float(t_end_s)
...
def evaluate_by_phase(aligned_csv: str, segs: list, name: str, include_phases=None) -> pd.DataFrame:
    ...
    if not rows:
        print(f"Warning: No segments included for estimator '{name}' after filtering by phases {include_phases}")
...
if __name__ == "__main__":
    ...
    # Determine common evaluation horizon (same as before)
    ...
    # Print segment boundaries once (sanity)
    for ph, t0, t1 in segs:
        print(f"Phase {ph:>2} ({PHASE_NAME.get(ph,'unknown'):>10}): {t0:8.2f}s -> {t1:8.2f}s | dur={t1-t0:6.2f}s")
    ...
    if out.empty:
        ...
        cols = ["phase", "phase_name", "estimator", "duration_s", "n",
                "ate_rmse_m", "yaw_abs_mean_rad", "yaw_abs_max_rad"]
