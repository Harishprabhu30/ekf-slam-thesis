...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--include",
        default="1,2,3,4",
        help="Comma-separated phase ids to include"
    )
    ...
    out_o = evaluate_by_phase(args.orb, segs, "orb", include_phases=include_set)

    out = pd.concat([out_w, out_e, out_o], ignore_index=True)

    os.makedirs(os.path.dirname(args.out), exist_ok=True)
    out.to_csv(args.out, index=False)

    if out.empty:
        print("Warning: no metrics to report.")
    else:
